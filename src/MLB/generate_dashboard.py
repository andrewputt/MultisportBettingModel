...
The dashboard is written to:
  data/processed/dashboard.html   ← open directly in any browser
"""
...
# Look for the template in the project root first, then next to this script
_template_candidates = [
    ROOT / "dashboard_template.html",                          # project root (shared)
    Path(__file__).resolve().parent / "dashboard_template.html",  # src/MLB/
]
TEMPLATE_HTML = next((p for p in _template_candidates if p.exists()), _template_candidates[0])
OUTPUT_HTML   = PROCESSED_DIR / "dashboard.html"
...
def build_meta(rows: list[dict], summary: dict, run_date: str) -> dict:
    ...
    """
    edges      = [r["mktProb"] and r["modelProb"] and (r["modelProb"] - r["mktProb"]) for r in rows]
...
def main() -> None:
    ...
    if not TEMPLATE_HTML.exists():
        print(
            f"[generate_dashboard] WARNING: template not found at {TEMPLATE_HTML}.\n"
            "  Picks saved. Copy src/MLB/dashboard_template.html to generate the HTML dashboard."
        )
        return
    ...
    html = inject(template, rows, meta)
    ...
    print(
        f"[generate_dashboard] {len(rows)} rows  |  {plays} play(s) ≥8% EV  |  avg edge {meta['avgEdge']}%\n"
